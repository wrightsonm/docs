...
IPAddress&#60;x><a class="cmnd" id="ipaddress"></a>|Set networking IP (`XXX.XXX.XXX.XXX`) addresses<BR>`IPAddress1` to set device IP address<BR><li>`0.0.0.0` to use dynamic IP address (DHCP)</li><li>`XXX.XXX.XXX.XXX` to set static IP address</li>`IPAddress2` to set gateway IP address<BR>`IPAddress3` to set subnet mask<BR>`IPAddress4` to set DNS server IP address<BR><BR>`IPAddress5` to set Secondary DNS server IP address<BR><BR>*follow IPAddress commands with `restart 1` to apply changes*
